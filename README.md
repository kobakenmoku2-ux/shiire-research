...
- 送料は、ヤフオクのみ一覧から送料込み/別を自動判定できる。ラクマ・メルカリは判定できないため「要確認」として扱い、通知にその旨を明記する(基本的に送料込みの出品が多いという実態に合わせ、計算上は「設定」タブで指定した金額(デフォルト0円)を使う)
...
「設定」タブ(販売手数料率・デフォルト送料)は、管理画面の「設定」タブを開いて保存すると自動で作成されるので、手動で作る必要はありません。
